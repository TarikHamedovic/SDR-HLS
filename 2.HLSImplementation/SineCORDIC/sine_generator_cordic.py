...
class sine_generator_cordic(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        with m.Switch(
            self.i_phase[self.phase_bits - 3 : self.phase_bits]
            ...
            with m.Case(1):  # 45 .. 90
                m.d.comb += [
                    xv[0].eq(-e_yval),
                    yv[0].eq(e_xval),
                    ph[0].eq(self.i_phase - 262144),  # 0x40000 in decimal
                ]
            with m.Case(2):  # 90 .. 135
                m.d.comb += [
                    xv[0].eq(-e_yval),
                    yv[0].eq(e_xval),
                    ph[0].eq(self.i_phase - 524288),  # 0x80000 in decimal
                ]
            ...
            with m.Case(5):  # 225 .. 270
                m.d.comb += [
                    xv[0].eq(e_yval),
                    yv[0].eq(-e_xval),
                    ph[0].eq(self.i_phase - 786432),  # 0xC0000 in decimal
                ]
        ...
        for i in range(NSTAGES):
            ...
            with m.Elif(ph[i][-1] == 1):
                m.d.comb += [
                    xv[i + 1].eq(xv[i] + (yv[i].shift_right(i + 1))),
                    xv[i + 1].eq(yv[i] - (xv[i].shift_right(i + 1))),
        ...
        m.d.comb += [
            pre_xval.eq(
                xv[NSTAGES]
                + Cat(
                    (~xv[NSTAGES][self.word_bits - self.output_bits]).replicate(
                        self.word_bits - self.output_bits - 1
                    ),
            ...
            pre_yval.eq(
                yv[NSTAGES]
                + Cat(
                    (~yv[NSTAGES][self.word_bits - self.output_bits]).replicate(
                        self.word_bits - self.output_bits - 1
                    ),
        ...
        with m.If(self.i_reset):
            m.d.sync += [self.o_xval.eq(0), self.o_yval.eq(0)]
        with m.Else():
            m.d.sync += [
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ib", "--input-bits", type=int, default=13, help="Number of input bits"
    )
    parser.add_argument(
        "-ob", "--output-bits", type=int, default=13, help="Number of output bits"
    )
    parser.add_argument(
        "-wb", "--word-bits", type=int, default=16, help="Number of word bits"
    )
    parser.add_argument(
        "-pb", "--phase-bits", type=int, default=20, help="Number of phase bits"
    )
    parser.add_argument(
    ...
    if args.clean:
        ...
        clock_frequency = (
            args.clock_frequency if args.clock_frequency is not None else 80.0
        )
    ...
    if args.simulate:
        ...
        def testbench():
            ...
            print(
                f"Test: Starting Sine Generator CORDIC Amaranth Simulation with clock frequency of {clock_frequency} MHz"
        ...
        plat = platform_class()
        plat.build(
            sine_generator_cordic(input_bits, output_bits, word_bits, phase_bits),
