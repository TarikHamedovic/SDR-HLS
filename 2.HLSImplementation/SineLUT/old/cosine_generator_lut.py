...
"""
-----------------------------------------------------------------------------
HLS Implementation for cosine generator using LUT
-----------------------------------------------------------------------------
"""
...
def cosine_table_generator(lgtable, ow, pb):
    ...
    for k in range(tbl_entries):
        ...
        dv = int(maxv * np.cos(ph)) & ((1 << ow) - 1)
        dv_values.append(dv)

    return dv_values

class cosine_generator_lut(Elaboratable):
    def __init__(self, CB=12, LB=8, PB=64):
        # Define the module's ports

        # Parameters
        # CB -> Cosine Bits , LB -> LUT Bits, PB -> Phase Bits
        self.CB = CB
        self.LB = LB
        self.PB = PB
        ...
        # Outputs
        self.cosinewave = Signal(signed(CB))
    ...
    def elaborate(self, platform):
        ...
        cosinewave_lut = cosine_table_generator(self.LB, self.CB, self.PB)
        # Initializing memory
        m.submodules.memory = memory = Memory(shape=self.CB, depth=2**self.LB, init=cosinewave_lut)
        rd_port = memory.read_port()

        m.d.comb += rd_port.addr.eq(phase[self.PB-self.LB:])
        m.d.comb += self.cosinewave.eq(rd_port.data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-cb", "--cosine-bits", type=int, default=12, help="Number of cosinewave bits")
    ...
    parser.add_argument("-df", "--desired-frequency", type=float, default=1.25, help="Desired frequency of generated cosinewave in MHz")
    ...
    parser.add_argument("-v",  "--verilog", action="store_true", help="Generate Verilog for cosine_generator_lut")
    ...
    if args.clean:
        ...
        cosine_bits = args.cosine_bits if args.cosine_bits is not None else 12
    ...
    if args.plot:
        ...
        cosinewave_values = cosine_table_generator(lut_bits, cosine_bits, phase_bits)
        signed_cosinewave_values = convert_to_signed(cosinewave_values, cosine_bits)
        plt.figure()
        plt.plot(signed_cosinewave_values)
        plt.title("Generated Cosine Wave")
        plt.xlabel("Sample")
    ...
    if args.simulate:
        ...
        async def testbench(ctx):
            print(f"Test: Starting Cosinewave LUT Amaranth Simulation with clock frequency of {clock_frequency} MHz")
        ...
        # Instantiate the cosine_generator_lut module
        dut = cosine_generator_lut(cosine_bits, lut_bits, phase_bits)
        ...
        plat = platform_class()
        plat.build(cosine_generator_lut(cosine_bits, lut_bits, phase_bits), do_program=do_program)
    elif args.verilog: 
        top = cosine_generator_lut(cosine_bits, lut_bits, phase_bits)
